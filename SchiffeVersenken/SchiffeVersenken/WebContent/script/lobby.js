...
/**
 * Der Splitter f�r die Methode onmessage in der Funktion websocket
 */
var DELIMITER = "%";

/**
 * Dieser Parameter gibt an, auf welchem Server der Websocket l�uft
 ...
 * Startet den Websocket und den Eventlistener f�r die Returntaste
 * @see read()
 * @see websocket()
 */
function onload() {
	...
	// TODO: Testen ob diese Funktion �berhaupt gebraucht wird. 
	read();
};

/**
 * �berwacht, ob die Returntaste gedr�ckt wurde. Returntastekeycode: 13
 * @see sendMessage()
 */
...
function websocket() {
	if ( "WebSocket" in window ) {
		...
		webSocket = new WebSocket( 'ws://' + SERVERIP + ":" + PORT + '/SchiffeVersenken/WebSocket/anything' ); 
					...
					// Wird beim �ffnen aufgerufen. �berpr�ft ob �berpr�ft ob die UserID oder der UserName im LocalSotrage ist.
					webSocket.onopen = function( event ) {
						
						// True wenn es schon eine UserID im LocalStorage gibt
						if (UserIDExists()) {
							
							// Dem Server wird die PlayerID gesendet und dem Websocket wird anhand der PlayerID der Player zuge�rdnet
							webSocket.send("3" + DELIMITER + getUserID() + DELIMITER + getUserName()); 
							
						} else {
							
							// Dem Server wird der Username �bermittelt und ein neues Playerobjekt wird erstellt
...
/**
 * �berpr�ft eine Nachricht auf ihren Inhalt (darf nicht leer sein oder nur aus leerzeichen bestehen) und sendet sie dem Websocket
 */
function sendMessage() {

		// TODO: Es k�nnen noch leere Nachrichten abgeschickt werden. Sofern +1 Leerzeichen geschrieben wird.
...
/**
 * �berpr�ft ob der Benutzername im LocalSotrage eingetragen ist <br />
...
/**
 * L�d den Benutzernamen aus dem LocalStorage <br />
...
/**
 * L�d die PlayerID aus dem Localstorage
...
/**
 * �berpr�ft den LocalStorage, ob die PlayerID leer ist. LS == "" <br />
 ...
 * �bertr�gt an den Websocket die UserID und der Spielname <br />
